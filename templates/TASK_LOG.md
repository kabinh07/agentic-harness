...
> this before planning; `/agentic-harness:run-manager` appends new tasks here — every task
...
> block, e.g. `/agentic-harness:clickup-log`) mirrors every write to the external tracker,
...
## Source convention
`Manager YYYY-MM-DD` (from /agentic-harness:run-manager) · `User YYYY-MM-DD` (direct ask) · `Auto YYYY-MM-DD` (self-queued)
